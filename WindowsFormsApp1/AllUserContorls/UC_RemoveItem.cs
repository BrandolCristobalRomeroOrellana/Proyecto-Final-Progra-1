...
namespace WindowsFormsApp1.AllUserContorls
{
    public partial class UC_RemoveItem : UserControl
    {
        ...
        {
            DelLabel.Text = "¿Como retirarlo?";
            DelLabel.ForeColor = Color.HotPink;
            loadData();
        ...
        {
            if (MessageBox.Show("Desea Eliminar el Producto", "Aviso Importante", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
            {
                int id = int.Parse(guna2DataGridView1.Rows[e.RowIndex].Cells[0].Value.ToString());
                query = "delete from items where iid =" + id + "";
                fn.setData(query, "Producto eliminado exitosamente.");
                loadData();
            }
        }
        ...
        {
            DelLabel.ForeColor = Color.HotPink;
            DelLabel.Text = "Haga clic en la fila para retirar el producto";
